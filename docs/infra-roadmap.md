...
| Backlog services Azure avancés (SQL Managed Instance, Synapse Link, Databricks, Data Lake, ExpressRoute, Private Link étendu, Traffic Manager, Front Door, Application Gateway, CDN, HPC Cache, API Management, Service Bus, Cache for Redis, Logic Apps, versionnement API/app) | Non spécifié | **Ventilé en session (septembre 2026)**, pas de sprint fourre-tout unique. Logique retenue : rattacher chaque service au sprint où il a un rôle réel, plutôt que de gonfler un sprint générique de découverte techno. (1) **Service Bus, Cache for Redis, API Management, Logic Apps, versionnement API/app rejoignent le Sprint 8 existant** (Step 16 novies) — ce sont des briques du quotidien microservices (messagerie managée, cache partagé, gateway API, orchestration légère), pas des sujets à part ; Service Bus y est le pendant managé Azure de Kafka (Step 16 quater), pas un remplacement. (2) **Synapse Link, Databricks, Data Lake, SQL Managed Instance/Hyperscale ouvrent un nouveau Sprint 12 — Plateforme données & analytics** (Step 16 decies) : aucun sprint existant n'a de contenu analytics, ça ne se rattache donc à rien de force. (3) **ExpressRoute, Private Link étendu, Traffic Manager, Front Door, Application Gateway, CDN, HPC Cache ouvrent un nouveau Sprint 13 — Edge, réseau global & résilience multi-cloud** (Step 16 undecies), qui recoupe directement le chantier warm standby Azure↔AWS de la ligne précédente (Traffic Manager y a déjà un rôle identifié). **Deux réserves documentées plutôt que passées sous silence** : ExpressRoute suppose une connectivité on-prem qui n'existe pas dans ce projet (100 % cloud, aucun datacenter à relier) — retenu quand même pour la couverture pédagogique du sujet, avec un scénario de test (ex. simulateur on-prem) à définir en Sprint 13, pas comme un besoin métier réel ; HPC Cache cible des charges de calcul haute performance que ce produit n'a pas — même réserve. Les deux nouveaux sprints sont volontairement placés **après** le Sprint 11 (TOGAF) dans la numérotation mais leur exécution réelle peut être réordonnée une fois le Sprint 11 entamé, puisqu'ils n'ont pas de dépendance dure envers lui. |
...
| 8 | Microservices **+ Kafka & patterns distribués (16 quater), contrats d'API & résilience (16 quinquies), traçabilité distribuée (16 octies), écosystème Azure microservices — Service Bus, Redis, API Management, Logic Apps (16 novies)** | Structurizr Lite + Mermaid (C4), OpenAPI + Redocly par service, Schema Registry, Pact, Polly, OpenTelemetry | ⏳ À venir |
...
| 12 | **Plateforme données & analytics Azure** — SQL Managed Instance, Synapse Link, Databricks, Data Lake (Step 16 decies) | — | ⏳ À venir (backlog, sept. 2026) |
| 13 | **Edge, réseau global & résilience multi-cloud** — ExpressRoute, Private Link étendu, Traffic Manager, Front Door, Application Gateway, CDN, HPC Cache (Step 16 undecies) | — | ⏳ À venir (backlog, sept. 2026) |
...
- [x] **Durée de rétention** définie par catégorie (§2 : backups 6j AWS *(relevé de 1j, plafond Free Tier réel confirmé en direct le 07/09/2026)*/7j Azure, logs 30j des deux côtés), **et purge automatisée par catégorie métier — décidée et implémentée cette semaine** (ADR-0012) : seuil 3 ans d'inactivité (dernière commande, ou création du compte à défaut), anonymisation. Mécanisme asymétrique par cloud : `CustomerRetentionPurgeHostedService` (BackgroundService in-process, `ArkCloud.API`) côté Azure, Lambda planifiée `modules/aws/gdpr-purge` côté AWS — même contrainte réseau Azure qu'ADR-0010, voir cette ADR pour le détail. Reste ouvert : `terraform apply` côté AWS pas encore exécuté par l'utilisateur pour provisionner la Lambda
- [~] **Droit à l'effacement** : chemin réel existe (`CustomerAppService.DeleteAsync`) — hard delete si pas de commande, anonymisation si des commandes existent (corrigé cette semaine, voir §3 : FK `ON DELETE RESTRICT` ajoutée, `Customer.Anonymize()`). Reste ouvert : jamais testé de bout en bout comme une vraie procédure formelle, et la survie en backup jusqu'à 7j (Azure) reste un fait à documenter plutôt qu'un problème à corriger
...
- [x] **Chiffrement au repos** — confirmé actif des deux côtés (clé managée, pas de CMK) ; pas de déclencheur identifié pour une CMK à ce niveau de classification, voir §6

### Chaîne d'approvisionnement logicielle

Trivy, Snyk et Checkov scannent **ce qu'on écrit**. Rien ne prouve **ce qu'on livre** ni **qui l'a produit**.

- [x] **SBOM** (Syft, via `anchore/sbom-action`) généré à chaque build (`api` et `web`) et publié comme artefact GitHub Actions (`sbom-api`/`sbom-web`) — répond à « quelles bibliothèques exactes tournent en production, à cette version d'image précise ? », question impossible à traiter après coup lors d'une CVE critique. Ajouté aux deux `publish-image` (`arkcloud-backend-ci.yml`, `arkcloud-frontend-ci.yml`), pas encore vérifié par un run réel (première PR/tag qui touche `publish-image` le confirmera)
- [x] **Signature des images** (Cosign, signature keyless via OIDC — cohérent avec l'authentification CI déjà en place, aucune clé à générer/stocker/tourner) — signe le digest exact résolu après le `docker push`, pas un tag mutable. L'attestation SBOM (`cosign attest --type spdxjson`) est aussi attachée à ce même digest, donc récupérable depuis le registre plutôt que depuis un artefact de run. Limite connue : signé côté GHCR uniquement — le digest de manifeste est identique côté ECR (le nom de dépôt n'entre pas dans le hash), mais la signature Cosign elle-même n'est pas répliquée vers ECR ; pas bloquant tant qu'Azure/GHCR reste le point de vérité, à revisiter si une vérification à l'admission ECS est un jour souhaitée
- [ ] **Vérification à l'admission** (Sprint 9) : la policy OPA/Gatekeeper refuse toute image non signée par le pipeline. C'est ce qui transforme la signature d'une formalité en contrôle réel — commande de vérification déjà documentée en commentaire dans les workflows (`cosign verify --certificate-identity-regexp ...`)
...
## Step 16 novies — Écosystème Azure microservices *(Sprint 8)*

> Rattaché au Sprint 8 plutôt qu'à un sprint dédié : ce sont des briques du quotidien microservices (messagerie, cache, gateway, orchestration), pas des sujets à part. Service Bus est le pendant managé Azure de Kafka (Step 16 quater) — les deux ne se remplacent pas, ils coexistent (Kafka comme log d'événements inter-services principal, Service Bus pour les scénarios où une file/topic managé Azure a du sens, ex. intégration avec d'autres services Azure PaaS).

- [ ] **Azure Service Bus** (topics/subscriptions) : cas d'usage à trancher avant implémentation — soit un flux spécifique qui ne justifie pas Kafka (volume faible, pas besoin de rejouabilité), soit un doublon pédagogique à assumer comme tel. Ne pas l'introduire juste pour cocher la case
- [ ] **Azure Cache for Redis** : cache partagé entre instances applicatives — candidat naturel pour les données de session/panier et pour soulager PostgreSQL Burstable sous charge (point dur déjà identifié Step 16 octies, autoscaling)
- [ ] **Azure API Management** : gateway API devant les microservices — rate limiting centralisé (trou STRIDE déjà identifié Step 16 septies), et point d'application du **versionnement d'API** (URL ou header, à trancher en ADR) plutôt que le versionnement actuellement absent
- [ ] **Versionnement des API/apps** : décision ADR à prendre indépendamment d'APIM — stratégie de versionnement (semver côté contrats OpenAPI, breaking change = nouvelle version majeure exposée), et politique de dépréciation des anciennes versions plutôt qu'une coupure sèche
- [ ] **Azure Logic Apps** : orchestration low-code pour des workflows cross-service simples (ex. notification après commande) — à évaluer face à un simple consommateur Kafka/Service Bus ; ne pas dupliquer une logique déjà couverte par les patterns Step 16 quater

## Step 16 decies — Plateforme données & analytics Azure *(Sprint 12, backlog)*

> Aucun sprint existant n'a de contenu analytics — ce n'est pas un manque comblé, mais une extension délibérée du périmètre du projet au-delà du besoin métier immédiat d'ArkCloud (OLTP e-commerce simple). Valeur pédagogique assumée plutôt que justifiée par un besoin réel identifié.

- [ ] **Azure Data Lake Storage (Gen2)** : socle du sprint — destination des données brutes/historisées, préalable aux deux items suivants
- [ ] **Azure Synapse Link** : réplication quasi temps réel depuis PostgreSQL Flexible Server (ou Cosmos DB si introduit) vers Synapse, sans impacter la charge OLTP — démontrer le découplage opérationnel/analytique
- [ ] **Azure Databricks** : traitement/transformation à grande échelle sur les données du Data Lake — un pipeline concret (ex. agrégats de ventes) plutôt qu'un cluster vide
- [ ] **Azure SQL Managed Instance / Hyperscale** : évaluer comme alternative de scaling à PostgreSQL Flexible Server plutôt que l'introduire en parallèle sans rôle clair — soit un ADR qui documente pourquoi rester sur Postgres, soit un scénario de migration limité (ex. réplica lecture seule pour Synapse)
- [ ] **Réplication logique PostgreSQL cross-cloud Azure→AWS (primaire→secondaire)** — chantier bloquant #1 de l'**ADR-0005** (primaire + DR, warm standby), rattaché ici plutôt qu'à un sprint réseau : c'est un sujet donnée, pas transport. Prérequis pour que la cible warm standby cesse d'être théorique

## Step 16 undecies — Edge, réseau global & résilience multi-cloud *(Sprint 13, backlog)*

> Recoupe directement le chantier warm standby Azure↔AWS identifié comme bloquant (journal de décisions, ligne « Architecture cible Azure/AWS ») — Traffic Manager y a déjà un rôle assigné, ce sprint est aussi l'endroit naturel pour l'exécuter.

- [ ] **Azure Traffic Manager** : couche DNS/health-check au-dessus d'Azure et AWS — chantier bloquant #2 de l'**ADR-0005** (primaire + DR, warm standby). Réplication PostgreSQL cross-cloud (bloquant #1, Sprint 12) et unification JWT (bloquant #3, hors sprint — voir ADR-0004) restent hors périmètre réseau de ce sprint
- [ ] **Azure Front Door** : point d'entrée global HTTP(S) + WAF — recouvrement partiel avec Traffic Manager (couche 7 vs DNS) à clarifier par un ADR plutôt que déployer les deux sans arbitrage
- [ ] **Azure CDN** : mise en cache des assets statiques Angular (Sprint 7) en périphérie — dépendance naturelle sur Front Door plutôt qu'un service isolé
- [ ] **Azure Application Gateway** : L7 + WAF régional — à positionner par rapport à Front Door (global) : Application Gateway en interne/régional si un scénario le justifie, sinon ADR documentant pourquoi Front Door seul suffit
- [ ] **Azure Private Link étendu** : le projet utilise déjà des endpoints privés (Postgres, Key Vault) — étendre systématiquement à tout nouveau service PaaS introduit dans ce sprint (Synapse, Service Bus, Redis) plutôt que de le limiter aux services historiques
- [ ] **Azure ExpressRoute** — ⚠️ réserve documentée : suppose une connectivité on-prem qui n'existe pas dans ce projet (100 % cloud). Retenu pour la couverture pédagogique du sujet ; nécessite de définir un scénario de test (ex. simulateur on-prem/VPN) avant toute implémentation, sinon le sujet reste théorique
- [ ] **Azure HPC Cache** — ⚠️ même réserve : cible des charges de calcul haute performance que ce produit n'a pas. À n'implémenter que si le Sprint 12 (Databricks) fait émerger un besoin I/O réel qui le justifie ; sinon documenter en ADR pourquoi le sujet reste en survol théorique plutôt que déployé

> **Chantier bloquant #3 (ADR-0005) volontairement hors de ce sprint** : l'unification de la clé de signature JWT entre Key Vault et Secrets Manager n'est pas un sujet réseau — elle recoupe ADR-0004 (gestion de `Jwt:Key`) et reste un item indépendant, sans sprint numéroté, à traiter quand la bascule primaire→DR devient concrète (une fois les bloquants #1 et #2 ci-dessus avancés).
...
- [x] SBOM (Syft) généré à chaque build + images signées (Cosign, keyless) — voir §"Chaîne d'approvisionnement logicielle" ci-dessus. Reste ouvert : vérification à l'admission *(Sprint 9)*
...
- [x] Classification des données par sensibilité (table et colonne) — `docs/rgpd-classification-donnees.md` §1
- [x] Durées de rétention définies et purge automatisée — ADR-0012, voir §"RGPD" plus haut dans ce document
- [~] Droit à l'effacement testé — y compris dans les sauvegardes et les logs — chemin corrigé (anonymisation si commandes, hard delete sinon), pas encore exercé comme une procédure formelle de bout en bout
...
- [x] Rotation des secrets — **mots de passe Postgres : rotation automatique tous les 90 jours sur les deux clouds** (Sprint 6, `modules/azure/secret-rotation` via Automation Runbook + schedule, `modules/aws/secret-rotation` via Secrets Manager + Lambda custom). **`GHCR_PAT` : étapes 2 à 5 automatisées** (Sprint 6, `ArkCloudInfra/scripts/rotate-ghcr-pat.ps1` — seule la génération du token, étape 1, reste manuelle sur github.com puisqu'aucune API GitHub ne le permet), ce qui a nécessité d'élargir le job `apply` de `terraform-ci.yml` pour accepter `workflow_dispatch` (avant ça, un re-run manuel ne faisait tourner que le plan, jamais l'apply réel). `Jwt:Key` reste manuel — pas juste "pas encore fait" mais délibérément non automatisé tant que le support multi-clés n'existe pas (voir backlog ci-dessous), puisque l'automatiser tel quel programmerait des déconnexions de masse récurrentes. Les deux restent couverts par les rappels d'échéance automatisés (`.github/workflows/secret-expiry-check.yml` + `.github/secrets-inventory.json`). Détail complet README §10 (ArkCloudInfra).
- [ ] **Backlog — éliminer le PAT `GHCR_PAT` structurellement** *(discuté en session, pas encore planifié sur un sprint précis)* : provisionner un Azure Container Registry et donner aux deux App Services un accès via identité managée (`AcrPull`), à la place de GHCR + PAT — même logique que le couple AWS ECR + rôle IAM déjà en place, qui n'a justement pas ce problème de rotation. Supprime le besoin de rotation plutôt que de l'automatiser, au prix d'une infra nouvelle (ACR, coût) et d'un double push d'image (GHCR pour AWS/ECS, ACR pour Azure) côté `arkcloud-backend-ci.yml`. Moins urgent maintenant que la rotation elle-même est scriptée.
- [ ] **Backlog — support multi-clés (`kid`) pour `Jwt:Key`** *(discuté en session, pas encore planifié sur un sprint précis)* : prérequis avant de pouvoir automatiser la rotation du JWT signing key sans dégrader l'expérience utilisateur. Aujourd'hui une seule clé sert à signer **et** valider les tokens — chaque rotation invalide instantanément tous les access/refresh tokens actifs (ADR-0004, risque accepté consciemment). Le support multi-clés permettrait de signer les nouveaux tokens avec la nouvelle clé tout en continuant à accepter les anciens tokens (ancienne clé) jusqu'à leur expiration naturelle — zéro déconnexion forcée. Changement applicatif réel dans `ArkCloud.API` (Authentication + logique de validation JWT), pas un script d'ops comme pour `GHCR_PAT` — candidat pour Sprint 7/8, pas le Sprint 6 en cours (infra/STRIDE).
- [ ] **Backlog — diff perpétuel `docker_registry_password`** *(discuté en session, pas encore planifié sur un sprint précis)* : voir ArkCloudInfra README §10 pour l'explication complète (Azure ne renvoie jamais un champ sensible sur `GET`, donc le diff est cosmétique, jamais appliqué en pratique). Piste creusée en session : `lifecycle { ignore_changes = [site_config[0].application_stack[0].docker_registry_password] }` a la bonne syntaxe pour ne cibler que cet attribut (contrairement à ce qui avait été supposé initialement — confirmé via un fil HashiCorp Discuss, la syntaxe map `site_config["..."]` casse tout le bloc, mais la syntaxe bloc `site_config[0]....` ne cible bien que l'attribut visé). Reste un vrai blocage fonctionnel, pas de syntaxe : `ignore_changes` est définitif, donc ça casserait silencieusement l'étape 3 de la rotation `GHCR_PAT` (le nouveau token ne serait plus jamais réellement poussé après un `terraform apply`). Vrai fix identifié mais non trivial : sortir ce champ du `azurerm_linux_web_app` natif et le gérer via un appel API direct (provider `azapi` ou `null_resource` + `local-exec`), déclenché seulement quand le hash du secret change — découplé du diffing d'état standard d'azurerm.
...
- [~] Quality gate SonarCloud — steps `dotnet-sonarscanner begin/end` câblés dans `arkcloud-backend-ci.yml`, no-op tant que `SONAR_TOKEN` (secret) + `SONAR_ORGANIZATION`/`SONAR_PROJECT_KEY` (variables repo) ne sont pas posés — nécessite de créer un compte SonarCloud gratuit (repo public) avant activation. Ne couvre que le backend .NET pour l'instant, pas `frontend/ArkCloud.Blazor` (voir commentaire dans `arkcloud-frontend-ci.yml` — pas dupliqué avant la migration Angular du Sprint 7)
...
- [~] Snyk — steps câblés (dépendances .NET + scan d'image, en plus de Trivy) dans les deux workflows CI, no-op tant que `SNYK_TOKEN` n'existe pas — nécessite un compte gratuit sur snyk.io. IaC (Terraform) non couvert ici : Checkov le fait déjà côté `ArkCloudInfra`, pas de doublon ajouté
- [x] Renovate configuré (`renovate.json` à la racine — patchs NuGet/npm auto-mergés, Terraform jamais auto-mergé, alertes de vulnérabilité jamais auto-mergées). Reste une action utilisateur : installer la Renovate GitHub App sur le repo pour que la config prenne effet — un fichier de config seul ne suffit pas
...
**Premier run réel (essai 28j, `workflow_dispatch` sur `develop`)** : analyse complète des 4 assemblies backend, warning attendu et non bloquant sur l'assembly Blazor non construite dans ce job (confirmé le risque documenté en commentaire dans le workflow). Une quality gate par défaut échoue : `Critical Rules Violated` (1 règle) — `ND1400`/`ND1401` ("Avoid namespaces mutually dependent" / "dependency cycles") entre le namespace anonyme et `ArkCloud.API.Middlewares`, causé par `Program.cs` (top-level statements) référençant les middlewares. Root-caused : `Program` issu de top-level statements vit forcément dans le namespace global — impossible de lui donner un `namespace` explicite dans le même fichier (`CS8803`, vérifié), donc pas de fix côté code sans abandonner le style top-level statements. **Faux positif connu** de ce pattern (partagé par d'autres analyseurs statiques comme SonarQube) : accepté tel quel, non corrigé. `stopIfQGFailed` volontairement non activé (voir step CI) donc ce résultat ne bloque pas le build. Deuxième finding, non bloquant (`Medium`, hors quality gate) : `ND2802`, l'assembly Serilog référencée en 3 versions différentes (`v4.3.0.0` direct, `v4.0.0.0` via `Serilog.Formatting.Compact`, `v4.2.0.0` via `Serilog.Extensions.Logging`) — pas encore investigué, à regarder si NDepend est adopté au-delà de l'essai.

### 18.3 Dépendances & sécurité — Sprint 6

- **Snyk** *(free tier)* : dépendances NuGet/npm, images Docker — vient en complément de Trivy (déjà en place côté scan d'image), pas en remplacement. IaC (Terraform) laissé à Checkov (`ArkCloudInfra`), pas dupliqué. Steps CI câblés cette semaine dans `arkcloud-backend-ci.yml`/`arkcloud-frontend-ci.yml`, `continue-on-error: true` tant que les résultats n'ont pas été triés au moins une fois — no-op sans `SNYK_TOKEN`.
- **Renovate** *(gratuit)* : mises à jour de dépendances automatisées, plus configurable que Dependabot (groupement de PRs, schedules, auto-merge sur les patchs mineurs). `renovate.json` ajouté cette semaine à la racine du repo — reste à installer la Renovate GitHub App pour que la config s'applique réellement.
