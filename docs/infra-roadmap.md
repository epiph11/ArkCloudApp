...
- [ ] **Backlog — réduire la partie manuelle de la rotation `GHCR_PAT`** *(discuté en session, pas encore planifié sur un sprint précis)* : GitHub n'expose aucune API pour créer un PAT (classique ou fine-grained) — seule la génération elle-même (étape 1/5, README §10) doit rester un humain sur github.com/settings/tokens, quelle que soit l'option retenue. Deux pistes, non exclusives, à trancher le moment venu :
  - **Automatiser les étapes 2 à 5** : un script/workflow qui prend le nouveau token en entrée et enchaîne seul `gh secret set` → `terraform apply` → `az webapp restart` (x2) → mise à jour de `expires_on` dans `secrets-inventory.json`. Réduit la procédure à 1 étape manuelle au lieu de 5, sans nouvelle infra.
  - **Éliminer le PAT structurellement** : provisionner un Azure Container Registry et donner aux deux App Services un accès via identité managée (`AcrPull`), à la place de GHCR + PAT — même logique que le couple AWS ECR + rôle IAM déjà en place, qui n'a justement pas ce problème de rotation. Supprime le besoin de rotation plutôt que de le rendre plus facile, au prix d'une infra nouvelle (ACR, coût) et d'un double push d'image (GHCR pour AWS/ECS, ACR pour Azure) côté `arkcloud-backend-ci.yml`.
