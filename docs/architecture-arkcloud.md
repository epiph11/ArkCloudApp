...
**SonarCloud — activé et vérifié en conditions réelles (12/09).** Organisation SonarQube Cloud créée (`epiph11`), projet `ArkCloud` importé, analyse automatique désactivée au profit du mode CI-based (nécessaire puisque `dotnet-sonarscanner begin/end` encadre déjà le build/test dans `arkcloud-backend-ci.yml`). `SONAR_TOKEN` (secret), `SONAR_ORGANIZATION`/`SONAR_PROJECT_KEY` (variables) posés sur le repo `ArkCloud`. Run `workflow_dispatch` déclenché en vérification : job `build-test` entièrement vert (2m44s), les trois étapes gated (`Setup Java`, `SonarCloud — begin analysis`, `SonarCloud — end analysis`) se sont réellement exécutées au lieu d'être no-op.

**Snyk — activé et vérifié en conditions réelles (12/09).** Organisation Snyk créée (`epiph11`), token API personnel généré et posé comme `SNYK_TOKEN` sur le repo `ArkCloud`. Run `workflow_dispatch` de vérification : l'étape "Snyk (dependencies)" (job `build-test`) s'est réellement exécutée (plus de "skipped") sur un job entièrement vert (3m01s). L'étape "Snyk (container image)" (job `publish-image`, après Trivy) reste à confirmer sur un run qui construit effectivement l'image Docker — pas déclenchée par `workflow_dispatch` seul sur `build-test`.

**Renovate — installé (12/09).** `renovate.json` déjà présent à la racine (patchs NuGet/npm auto-mergés, Terraform et alertes de vulnérabilité jamais auto-mergés). GitHub App installée via `github.com/marketplace/renovate` (l'URL `github.com/apps/renovate` renvoyait un 404 au moment de l'installation) sur les deux repos réels — `ArkCloudApp` (nom GitHub réel du dépôt applicatif, désigné `ArkCloud` dans ce document) et `ArkCloudInfra` — statut "Enabled" confirmé côté dashboard Mend/Renovate. Premier job pas encore exécuté ("Last Job Run" vide) au moment de cette vérification — normal juste après l'installation en plan Community, mode "Interactive".
